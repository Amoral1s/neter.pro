...
$compare_attribute_rows = isset($compare_attribute_rows) && is_array($compare_attribute_rows) ? $compare_attribute_rows : array();
$show_attribute_labels = !empty($show_attribute_labels);
...
<div <?php wc_product_class('compare-card swiper-slide', $product); ?> data-product-id="<?php echo esc_attr($product_id); ?>">
    <div class="compare-card__product">
        ...
        <a class="product-top" href="<?php echo esc_url(get_permalink($product_id)); ?>">
            <div class="product-thumb">
                <?php
                if (has_post_thumbnail($product_id)) {
                    echo get_the_post_thumbnail($product_id, 'woocommerce_thumbnail');
                } else {
            ...
            <div class="product-rating">
                ...
                echo $average ? wc_get_rating_html($average) : '<div class="star-rating"><span style="width:0%"></span></div>';
                ?>
            ...
            <b class="product-title"><?php echo esc_html($product->get_name()); ?></b>
        </a>
        <div class="product-bottom">
            <?php if ((float) $product->get_price() !== 1.0) : ?>
            ...
            foreach ($attributes as $attribute_slug => $icon_html) {
                $terms = wc_get_product_terms($product_id, $attribute_slug, array('fields' => 'names'));
    ...
    <div class="compare-card__attrs">
        <?php foreach ($compare_attribute_rows as $row) : ?>
            <?php
            $row_value = isset($row['values'][$product_id]) ? $row['values'][$product_id] : array('value' => '-', 'is_html' => false);
            $is_empty_value = trim(wp_strip_all_tags((string) $row_value['value'])) === '-';
            ?>
            <div class="compare-card__attr-row" data-compare-row-key="<?php echo esc_attr($row['key']); ?>" data-compare-row-different="<?php echo !empty($row['different']) ? '1' : '0'; ?>">
                <div class="compare-card__attr-label"><?php echo esc_html($row['label']); ?></div>
                <div class="compare-card__attr-value <?php echo $is_empty_value ? 'is-empty' : ''; ?>">
                    <?php
                    if (!empty($row_value['is_html'])) {
                        echo wp_kses_post($row_value['value']);
                    } else {
                        echo esc_html($row_value['value']);
                    }
                    ?>
                </div>
            </div>
